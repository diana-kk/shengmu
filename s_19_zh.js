...
var s_zhuozi = Paper.image('img/s_zhuozi.png', 0, 0, 120, 120).toDefs();
...
bottomArea.text(48, 180, 'zh').click(playAud01).attr({
  'font-size': 90,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_zh.wav');
  audio1.play();
}

s_zhuozi.use().transform('t125, 260').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['zhuōzi', '책상']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_zhuozi.wav');
  audio2.play();
}
