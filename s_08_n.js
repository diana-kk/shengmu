...
var s_niu = Paper.image('img/s_niu.png', 0, 0, 160, 120).toDefs();
...
bottomArea.text(65, 180, 'n').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_n.wav');
  audio1.play();
}

s_niu.use().transform('t110, 250').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['niú', '소']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_niu.wav');
  audio2.play();
}
