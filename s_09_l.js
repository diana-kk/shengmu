...
var s_liwu = Paper.image('img/s_liwu.png', 0, 0, 130, 120).toDefs();
...
bottomArea.text(80, 186, 'l').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_l.wav');
  audio1.play();
}

s_liwu.use().transform('t110, 250').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['lǐwù', '선물']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_liwu.wav');
  audio2.play();
}
