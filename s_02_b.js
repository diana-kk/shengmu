...
var s_bisa = Paper.image('img/s_bisa.png', 0, 0, 160, 120).toDefs();
...
bottomArea.text(65, 190, 'b').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_b.wav');
  audio1.play();
}

s_bisa.use().transform('t100, 240').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['bǐsà', '피자']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_bisa.wav');
  audio2.play();
}
