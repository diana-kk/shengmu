...
var s_gou = Paper.image('img/s_gou.png', 0, 0, 130, 140).toDefs();
...
bottomArea.text(65, 176, 'g').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_g.wav');
  audio1.play();
}

s_gou.use().transform('t120, 250').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['gǒu', '개']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_gou.wav');
  audio2.play();
}
