...
var s_xigua = Paper.image('img/s_xigua.png', 0, 0, 160, 130).toDefs();
...
bottomArea.text(66, 180, 'x').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_x.wav');
  audio1.play();
}

s_xigua.use().transform('t100, 245').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['xīguā', '수박']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_xigua.wav');
  audio2.play();
}
