...
var s_cha = Paper.image('img/s_cha.png', 0, 0, 130, 110).toDefs();
...
bottomArea.text(47, 180, 'ch').click(playAud01).attr({
  'font-size': 90,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_ch.wav');
  audio1.play();
}

s_cha.use().transform('t120, 265').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['chá', '차']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_cha.wav');
  audio2.play();
}
