...
var s_feiji = Paper.image('img/s_feiji.png', 0, 0, 150, 110).toDefs();
...
bottomArea.text(73, 188, 'f').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_f.wav');
  audio1.play();
}

s_feiji.use().transform('t110, 245').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['fēijī', '비행기']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_feiji.wav');
  audio2.play();
}
