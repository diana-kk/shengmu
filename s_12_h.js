...
var s_hai = Paper.image('img/s_hai.png', 0, 0, 140, 140).toDefs();
...
bottomArea.text(65, 185, 'h').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_h.wav');
  audio1.play();
}

s_hai.use().transform('t110, 235').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['hǎi', '바다']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_hai.wav');
  audio2.play();
}
