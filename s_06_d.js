...
var s_didi = Paper.image('img/s_didi.png', 0, 0, 130, 150).toDefs();
...
bottomArea.text(60, 186, 'd').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_d.wav');
  audio1.play();
}

s_didi.use().transform('t125, 235').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['dìdi', '남동생']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_didi.wav');
  audio2.play();
}
