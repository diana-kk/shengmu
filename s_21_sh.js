...
var s_shi = Paper.image('img/s_shi.png', 0, 0, 130, 110).toDefs();
...
bottomArea.text(50, 180, 'sh').click(playAud01).attr({
  'font-size': 90,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_sh.wav');
  audio1.play();
}

s_shi.use().transform('t120, 265').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['shí', '열, 10']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_shi.wav');
  audio2.play();
}
