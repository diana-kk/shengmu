...
var s_jidan = Paper.image('img/s_jidan.png', 0, 0, 140, 140).toDefs();
...
bottomArea.text(80, 182, 'j').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_j.wav');
  audio1.play();
}

s_jidan.use().transform('t110, 245').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['jīdàn', '달걀']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_jidan.wav');
  audio2.play();
}
