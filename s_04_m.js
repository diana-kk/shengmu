...
var s_mao = Paper.image('img/s_mao.png', 0, 0, 160, 120).toDefs();
...
bottomArea.text(52, 180, 'm').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_m.wav');
  audio1.play();
}

s_mao.use().transform('t105, 260').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['māo', '고양이']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_mao.wav');
  audio2.play();
}
