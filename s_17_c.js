...
var s_cidian = Paper.image('img/s_cidian.png', 0, 0, 140, 110).toDefs();
...
bottomArea.text(66, 180, 'c').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_c.wav');
  audio1.play();
}

s_cidian.use().transform('t125, 255').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['cídiǎn', '사전']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_cidian.wav');
  audio2.play();
}
