...
var s_tang = Paper.image('img/s_tang.png', 0, 0, 120, 140).toDefs();
...
bottomArea.text(72, 186, 't').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_s_t.wav');
  audio1.play();
}

s_tang.use().transform('t115, 240').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var tt = bottomArea.text(180, 480, ['táng', '사탕']).click(playAud02).attr({
  'font-size': 70,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_s_tang.wav');
  audio2.play();
}
